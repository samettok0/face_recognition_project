#!/usr/bin/env python3
"""
Button Trigger for Face Recognition Authentication with RFID Backup - HEADLESS VERSION
...
HEADLESS COMPATIBLE: Works without terminal/stdin - suitable for cron/service
"""
...
class FaceRecognitionButtonTriggerHeadless:
    def __init__(self, gpio_pin=16, buzzer_pin=26, debounce_time=0.5, cooldown_time=3.0):
        """
        Initialize the button trigger system with RFID backup - HEADLESS VERSION
        ...
        self.setup_rfid_monitoring_headless()
        ...
        print(f"RFID backup system initialized (HEADLESS MODE)")
    ...
    def setup_rfid_monitoring_headless(self):
        """Setup RFID input monitoring for headless operation"""
        def rfid_device_monitor():
            """Monitor /dev/input devices for RFID input"""
            print("🏷️ RFID monitoring started (headless mode)")
            
            # For headless operation, we'll use a different approach
            # This simulates RFID cards for now - in production you'd monitor USB HID device
            rfid_simulation_cards = [
                "1234567890",  # Simulated card 1
                "0987654321",  # Simulated card 2
            ]
            
            while True:
                try:
                    # In a real implementation, you would:
                    # 1. Monitor /dev/input/eventX for USB HID device
                    # 2. Parse keyboard events for 10-digit sequences
                    # 3. Put valid RFID data into queue
                    
                    # For now, we simulate by checking for specific trigger files
                    # You can create these files to simulate RFID scans in headless mode:
                    # echo "1234567890" > /tmp/rfid_scan
                    
                    rfid_trigger_file = Path("/tmp/rfid_scan")
                    if rfid_trigger_file.exists():
                        try:
                            rfid_data = rfid_trigger_file.read_text().strip()
                            if rfid_data and len(rfid_data) >= 8:
                                self.rfid_input_queue.put(rfid_data)
                                print(f"🏷️ RFID scan detected from trigger file: {rfid_data}")
                            rfid_trigger_file.unlink()  # Remove the trigger file
                        except Exception as e:
                            print(f"Error reading RFID trigger file: {e}")
        ...
        rfid_thread = threading.Thread(target=rfid_device_monitor, daemon=True)
        rfid_thread.start()
        
        print("💡 HEADLESS RFID NOTE:")
        print("   To simulate RFID scan: echo 'CARD_ID' > /tmp/rfid_scan")
        print("   Example: echo '1234567890' > /tmp/rfid_scan")
    ...
    def activate_rfid_backup(self):
        ...
        print("💡 HEADLESS MODE: echo 'YOUR_CARD_ID' > /tmp/rfid_scan")
    ...
    # All buzzer patterns remain the same
    ...
    def run(self):
        """Main run loop"""
        try:
            print("🚀 Face Recognition + RFID Backup System Running (HEADLESS)")
...
def main():
    ...
    # Initialize and run the button trigger with RFID backup (headless version)
    trigger = FaceRecognitionButtonTriggerHeadless(
        gpio_pin=16,
        buzzer_pin=26,
